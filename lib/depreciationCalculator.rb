...
module DepreciationCalculator
	def self.calculateDepreciatedValue(carDetails)
		...
		@estimated_value = ((age_dep[carDetails['age']].to_i * 0.5) + (kms_dep[carDetails['mileage']].to_i * 0.3) + (tyre_cond[carDetails['tyre']].to_i * 0.05) + (owners[carDetails['owner']].to_i * 0.15)) * carDetails['price'].to_i * 0.01
